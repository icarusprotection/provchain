...
def set(
    ...
    if "." not in key:
        console.print(
            "[red]Error: Key must be in format 'section.key' (e.g., 'general.threshold')[/red]"
        )
        raise typer.Exit(1)
    ...
    if section not in Config.DEFAULT_CONFIG:
        console.print(
            f"[red]Error: Invalid section '{section}'. Valid sections: {', '.join(Config.DEFAULT_CONFIG.keys())}[/red]"
        )
        raise typer.Exit(1)

    # Get the config instance - use the same path resolution as Config class
    config_path = Path.home() / ".provchain" / "config.toml"
    ...
    try:
        if expected_type is bool:
            ...
            if value.lower() in ("true", "1", "yes", "on"):
                ...
                console.print(
                    f"[red]Error: Invalid boolean value '{value}'. Use 'true' or 'false'[/red]"
                )
                raise typer.Exit(1)
            ...
            try:
                ...
                if value_cleaned.startswith("[") and value_cleaned.endswith("]"):
                    ...
                    try:
                        ...
                        converted_value = [
                            v.strip().strip('"').strip("'")
                            for v in value_cleaned.strip("[]").split(",")
                            if v.strip()
                        ]
                else:
                    # Comma-separated values
                    converted_value = [
                        v.strip().strip('"').strip("'") for v in value.split(",") if v.strip()
                    ]
        else:
            # String type
        ...
        if section == "general" and config_key == "threshold":
            ...
            if converted_value not in valid_thresholds:
                console.print(
                    f"[red]Error: Invalid threshold value '{converted_value}'. Must be one of: {', '.join(valid_thresholds)}[/red]"
                )
                raise typer.Exit(1)
        ...
        if section == "behavior" and config_key == "network_policy":
            ...
            if converted_value not in valid_policies:
                console.print(
                    f"[red]Error: Invalid network_policy value '{converted_value}'. Must be one of: {', '.join(valid_policies)}[/red]"
                )
                raise typer.Exit(1)
        ...
        if section == "output" and config_key == "format":
            ...
            if converted_value not in valid_formats:
                console.print(
                    f"[red]Error: Invalid format value '{converted_value}'. Must be one of: {', '.join(valid_formats)}[/red]"
                )
                raise typer.Exit(1)
        ...
        if section == "output" and config_key == "verbosity":
            ...
            if converted_value not in valid_verbosities:
                console.print(
                    f"[red]Error: Invalid verbosity value '{converted_value}'. Must be one of: {', '.join(valid_verbosities)}[/red]"
                )
                raise typer.Exit(1)
        ...
        console.print(
            f"[red]Error: Invalid value type. Expected {expected_type.__name__}, got '{value}'[/red]"
        )
        raise typer.Exit(1)
...
def show() -> None:
    ...
    if config_path.exists():
        ...
        console.print(
            "[yellow]No configuration file found. Run 'provchain config init' to create one.[/yellow]"
        )
